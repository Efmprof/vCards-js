...
(function vCardFormatter() {
    ...
    module.exports = {
        ...
        getFormattedString: function (vCard) {
            ...
            const encodingPrefix = majorVersion >= 3 ? '' : ';CHARSET=ISO-8859-1';
            ...
            if (vCard.namedUrl) {
                if (!Array.isArray(vCard.namedUrl)) {
                    formattedVCardString += 'URL;' + "TYPE=" + vCard.namedUrl.name + encodingPrefix + ':' + e(vCard.namedUrl.url) + nl();
                } else {
                    vCard.namedUrl.forEach((url) => {
                        formattedVCardString += 'URL;' + "TYPE=" + url.name + encodingPrefix + ':' + e(url.url) + nl();
                    });
                }
            }
